...
#https://www.datacamp.com/community/tutorials/R-nlp-machine-learning?tap_a=5644-dce66f&tap_s=10907-287229
...
#dim() function, you see that there are 7 columns and 824 observations
dim(prince)

#see one of the lyrics
...
###remove undesirable common words
...
prince_words_filtered <- prince %>%
  ...
  anti_join(stop_words) %>% #removes the stop words via antijoin using column word.
  distinct() %>%
...
#Table examining word race, limit top 10
...
#full word count group by the song/chart level
...
###Skewed to the right..check out top 10 with more than 800, included rap star
full_word_count %>%
...
#wordcloud2(prince_words_counts[1:300, ], figPath = "guitar.png", 
#           color = "random-dark", size = 1.5)

#letterCloud(prince_words_counts[1:300, ], word = "PRINCE")
...
###Keep the stop words and short words for quantitative purposes
...
#Lexical Density
#number of unique words divided by density
...
###Taking those previous charts and comparing to chart toppers
...
#TF-IDF = TF * IDF
# The assumption behind TF-IDF is that terms that appear more frequently in a document 
#should be given a higher weight, unless it also appears in many documents.
