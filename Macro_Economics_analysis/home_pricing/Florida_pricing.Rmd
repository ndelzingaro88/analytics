...
\fancypagestyle{firstpage}{
    ...
    \lfoot{\raisebox{16px}{\includegraphics[height = 8px]{~/git_dir/analytics/Macro_Economics_analysis/home_pricing/fsu_logo.png}}}
...
\fancypagestyle{contentpage}{
    ...
    \fancyfoot{}
    \lhead{
        \raisebox{-12px}{\includegraphics[height = 10px]{~/git_dir/analytics/Macro_Economics_analysis/home_pricing/florida_flag.png}
        {\hspace{10px}\fontsize{9pt}{8pt}\selectfont\fontseries{l}} {\color{orchardgreen}{\reportcategory{}}}}
    }
    \rhead{\raisebox{-12px}{\reportdate{}}}
    \lfoot{\raisebox{2px}{\includegraphics[height = 8px]{~/git_dir/analytics/Macro_Economics_analysis/home_pricing/fsu_logo.png}}}
...
df.ca <- filter(df.hpi,grepl("FL",place_name)| place_name=="Florida" , hpi_type=="traditional", hpi_flavor=="purchase-only")
...
```{r query_securitization_1, fig.height=6}
ggplot(data= df.ca %>% filter(level=="State"), aes(x=date,y=index_sa,group=place_name))+
  ...
  geom_text(data=. %>% tail(1), aes(label="FL"),color=rgb(103,180,75, maxColorValue = 256),fontface="bold",nudge_y=5)+
  geom_line(data=df.us %>% select(date,index_sa), aes(group="FL"),linetype=2, color=rgb(9, 177,240, maxColorValue = 256),size=1.05)+
  geom_text(data=df.us %>% select(date,index_sa) %>% tail(1), aes(group="FL",label="US"),nudge_y=-5, color=rgb(9, 177,240, maxColorValue = 256))+
  #theme_ridges(font_family="Roboto")+
  theme(text = element_text(color = "#27408b"))+
  labs(y="House Price Index: 1991Q1=100",x="date",
       title="House price index: FL vs US"
       #caption="@lenkiefer Source: FHFA purchase-only house price index, SA"
       )
```

&nbsp;
```{r loanlevel, fig.height=6}
ggplot(data= df.ca %>% filter(level=="State",year(date)>1991), aes(x=date,y=hpa4,group=place_name))+
  geom_line(color=rgb(103,180,75, maxColorValue = 256), size=1.05)+
  geom_text(data=. %>% tail(1), aes(label="FL"),color=rgb(103,180,75, maxColorValue = 256),fontface="bold",nudge_y=.01)+
  geom_line(data=filter(df.us,year(date)>1991) %>% select(date,hpa4), aes(group="FL"),linetype=2, color=rgb(9, 177,240, maxColorValue = 256),size=1.05)+
  geom_text(data=df.us %>% select(date,hpa4) %>% tail(1), aes(group="FL",label="US"),nudge_y=-.01, color=rgb(9, 177,240, maxColorValue = 256))+
  #theme_ridges(font_family="Roboto")+
  scale_y_continuous(labels=scales::percent)+
  theme(text = element_text(color = "#27408b"))+
  labs(y="4-quarter % change",x="date",
       title="House price appreciation: FL vs US",
...
ggplot(data= df.ca %>% filter(level=="MSA",year(date)>1991), aes(x=date,y=index_sa,group=place_name))+
  geom_line(color=rgb(103,180,75, maxColorValue = 256), size=1.05)+
  geom_line(data=df.ca %>% filter(level=="State",year(date)>1991) %>% select(date,index_sa), aes(group="FL"),linetype=2, color=rgb(9, 177,240, maxColorValue = 256),size=1.05)+
 # theme_ridges(font_family="Roboto")+
  theme(text = element_text(color = "#27408b"),strip.text=element_text(size=7))+
    labs(y="House Price Index: 1991Q1=100",x="date",
       title="House price index", subtitle="Solid line metro, dotted line FL"
       #caption="@lenkiefer Source: FHFA purchase-only house price index, SA"
       )+
...
```{r MonthsOffer, fig.height=5}
df.ca$place_name <- str_wrap(df.ca$place_name, width = 35)
...
make_plot_appear <- function(i){
  p <-
    ...
    labs(y="House Price Appreciation (y/y %)",x="date (quarterly)",
         title="House price appreciation: Florida",
         subtitle="4-quarter percent change in index"
         #caption="@lenkiefer Source: FHFA purchase-only house price index, SA"
         )
...
#   }
#   
# }, movie.name = "~/git_dir/analytics/Macro_Economics_analysis/FL_Pricing/hpa.gif", ani.width=700, ani.height=600,scale=1.2)  #set YOURDIRECTORY to where you want to save the file
```
